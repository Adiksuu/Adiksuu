<img width="1500" height="500" alt="banner" src="https://github.com/user-attachments/assets/260defc0-7384-48e1-a842-1bbbdd578042" />

# Hi, I'm Adiksuu

I'm a web developer from Poland focused on building modern, performant and well-designed web applications.

Currently I'm working mainly with Laravel, React and TypeScript, while exploring desktop development with Electron. I enjoy creating products from the ground up — from backend architecture to clean, intuitive user interfaces.

## Current Focus

- Building full-stack applications with Laravel and React
- Developing desktop applications using Electron
- Designing modern and responsive interfaces with Tailwind CSS
- Writing maintainable, scalable code
- Learning new technologies through personal projects

## Tech Stack

### Languages

![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-323330?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### Frameworks & Tools

![Laravel](https://img.shields.io/badge/Laravel-FF2D20?style=for-the-badge&logo=laravel&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Electron](https://img.shields.io/badge/Electron-191970?style=for-the-badge&logo=electron&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=for-the-badge&logo=tailwind-css&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)

### Development

![PhpStorm](https://img.shields.io/badge/PhpStorm-000000?style=for-the-badge&logo=phpstorm&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![NPM](https://img.shields.io/badge/NPM-CB3837?style=for-the-badge&logo=npm&logoColor=white)

## Featured Projects

- **Orbit** — modern project and workspace management platform built with Laravel and React. Comming this year!
- **JustTalk** — cross-platform messaging application available now on Android. Windows and Linux in the end of year.
- More projects are available in my repositories.

## GitHub Stats

![](https://github-readme-streak-stats.herokuapp.com/?user=Adiksuu&theme=react&hide_border=true)

![](https://github-readme-stats.vercel.app/api/top-langs/?username=Adiksuu&theme=react&hide_border=true&layout=compact)

## Find Me

[![Behance](https://img.shields.io/badge/Behance-1769FF?style=for-the-badge&logo=behance&logoColor=white)](https://behance.net/Adiksuu)
